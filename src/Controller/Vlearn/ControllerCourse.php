...
use Vittascience\Entity\Vlearn\Folders;
...
use Vittascience\Entity\Vclassroom\CourseLinkUser;
...
class ControllerCourse extends Controller
{
        ...
        $this->actions = array(
            'get_one' => function () {
                ...
                $activities = $this->entityManager
                    ->getRepository(CourseLinkActivity::class)
            ...
            'get_by_filter' => function () {
                ...
                // fetch data from db 
                $results = $this->entityManager->getRepository(Course::class)->getByFilter($sanitizedFilters, $search, $sort, $page);
            ...
            'count_by_filter' => function ($data) {
                ...
                // fetch data from db 
                return $this->entityManager->getRepository(Course::class)->countByFilter($sanitizedFilters,  $search);
            },
            'add' => function () {
            ...
            'increment_views' => function () {
                ...
                $tutorial = $this->entityManager->getRepository(Course::class)->findOneBy(array("id" => $tutorialId));
            ...
            'update' => function () {
                ...
                $databaseCourse = $this->entityManager->getRepository(Course::class)->findOneBy(array("id" => $tutorial->getId()));
                ...
                $lessonsDatabase = $this->entityManager->getRepository(Lesson::class)->findBy(array("tutorial" => $tutorial));
                foreach ($lessonsDatabase as $val) {
                    $this->entityManager->remove($val);
                }
                $activities = $this->entityManager->getRepository(CourseLinkActivity::class)
                ...
                $linkedDatabase = $this->entityManager->getRepository(CourseLinkCourse::class)->findBy(array("tutorial1" => $tutorial));
                ...
                //add parts to the tutorial
                for ($index = 0; $index < count($tutorialParts); $index++) {
                    $activity = new Activity($tutorialParts[$index]->title, $tutorialParts[$index]->content, $this->entityManager->getRepository(User::class)
                        ->find($this->user['id']));
            ...
            'delete' => function ($data) {
                ...
                $databaseCourse = $this->entityManager->getRepository(Course::class)->find($courseIdToDelete);
                ...
                $lessonsDatabase = $this->entityManager->getRepository(Lesson::class)->findBy(array("tutorial" => $databaseCourse));
                foreach ($lessonsDatabase as $val) {
                    $this->entityManager->remove($val);
                }

                // remove entries from "learn_favorites" table
                $favoriteDatabase = $this->entityManager->getRepository(Favorite::class)->findBy(array("tutorial" => $databaseCourse));
                foreach ($favoriteDatabase as $val) {
                    $this->entityManager->remove($val);
                }

                // remove entries from "learn_tutorials_link_tutorials" table
                $linkedCoursesAsMain = $this->entityManager->getRepository(CourseLinkCourse::class)->findBy(array('tutorial1' => $databaseCourse));
                foreach ($linkedCoursesAsMain as $linkedCourseAsMain) {
                    $this->entityManager->remove($linkedCourseAsMain);
                    $this->entityManager->flush();
                }
                $linkedCoursesAsRelated = $this->entityManager->getRepository(CourseLinkCourse::class)->findBy(array('tutorial2' => $databaseCourse));
    ...
    private function saveRelatedTutorialsIfNeeded($mainTutorial, $relatedTutorialsIds)
    {
        foreach ($relatedTutorialsIds as $relatedTutorialId) {
            $tutorial2Exists = $this->entityManager->getRepository(Course::class)->find($relatedTutorialId);

            if ($tutorial2Exists) {
                $linkedTutoRelationExists = $this->entityManager
                    ->getRepository(CourseLinkCourse::class)
                    ->findOneBy(array(
    ...
    private function saveLessonsIfNeeded($mainTutorial, $chapterIds)
    {
        ...
        foreach ($chapterIds as $chapterId) {
            $chapterFound = $this->entityManager->getRepository(Chapter::class)->find($chapterId);
