...
namespace Vittascience\Controller\Vuser;
...
use Vittascience\Entity\Vclassroom\Restrictions;
use Vittascience\Controller\Vclassroom\Controller;
...
use Vittascience\Entity\Vclassroom\UsersRestrictions;
use Vittascience\Entity\Vclassroom\ActivityLinkClassroom;
use Vittascience\Entity\Vuser\ClassroomUserConnectionLog; 
    ...
    public function __construct($entityManager, $user, $url = null)
    {
        ...
        $this->actions = array(
            ...
            'linkSystem' => function () {
                ...
                $classroom = $this->entityManager->getRepository(Classroom::class)->findOneBy(array("link" => $classroomLink));
                ...
                $classroomTeacher = $this->entityManager->getRepository(ClassroomLinkUser::class)
                ...
                $pseudoUsed = $this->entityManager->getRepository(User::class)->findBy(array('pseudo' => $pseudo));
                foreach ($pseudoUsed as $p) {
                    $pseudoUsedInClassroom = $this->entityManager->getRepository(ClassroomLinkUser::class)->findOneBy(array('user' => $p, 'classroom' => $classroom));
            ...
            'signup' => function ($data) {
                ...
                if (
                    ...
                    $user->setPassword(password_hash($password, PASSWORD_DEFAULT));
                    $lastQuestion = $this->entityManager->getRepository(User::class)->findOneBy([], ['id' => 'desc']);
