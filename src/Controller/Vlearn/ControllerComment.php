...
use Vittascience\Entity\Vlearn\Comment;
...
class ControllerComment extends Controller
{
    public function __construct($entityManager, $user)
    {
        ...
        $this->actions = array(
            'add' => function ($data) {
                $commentAnswered = $this->entityManager->getRepository(Comment::class)
                    ->find($data['comid']);
                $tutorial = $this->entityManager->getRepository(Course::class)
                    ->find($data['tutoid']);
                $comment = new Comment();
            ...
            'delete' => function ($data) {
                ...
                $comment = $this->entityManager->getRepository(Comment::class)->find($data['comid']);
            ...
            'alert' => function ($data) {
                $comment = $this->entityManager->getRepository(Comment::class)->find(intval($data['comid']));
                $mail = new Mailer();
    ...
    a fait une demande de modération concernant le message #" . $comment->getId() . ".</br><b>Raison :</b><br/>";
            ...
            'get_from_tutorials' => function ($data) {
                $comments = $this->entityManager->getRepository(Comment::class)->findBy(array("tutorial" => $data['tutorial_id']));
                $arrayResult = array();
            ...
            'get_multiple_users' => function () {
                ...
                $users = $this->entityManager->getRepository(User::class)->getMultipleUsers($_GET['ids']);
                $arrayResult = array();
